...
$service = @($serviceConfig.services | Where-Object { $_.project -eq 'LASEN' -and $_.id -eq 'frontend' })
if ($service.Count -ne 1 -or $null -eq $service[0].environment.VITE_PORT) {
    throw 'runtime-services.json 缺少 LASEN 前端的 VITE_PORT 配置。'
}
...
if ($env:VITE_PORT -ne "$port") {
    throw "LASEN 前端 VITE_PORT=$env:VITE_PORT 与服务地址端口 $port 不一致。"
}
$workingDirectory = 'D:\songgf\Projects\lasen-ui'
...
while ((Get-Date) -lt $deadline) {
    if (Test-Health) {
        Write-Output "LASEN 前端已就绪：$url（PID $($process.Id)）"
        return
    }
    if ($process.HasExited) {
        throw "LASEN 前端已退出，退出码：$($process.ExitCode)。"
    }
    Start-Sleep -Seconds 2
}

throw "LASEN 前端在 $HealthWaitSeconds 秒内未就绪，PID：$($process.Id)。"
