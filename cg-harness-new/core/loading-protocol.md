...
9. 能力注册表存在 `deep_skill` 时，在该能力的触发条件满足后，先读取 capability 文件，再读取对应 `skills/<名称>/SKILL.md`；能力文件负责框架门禁，技能负责细化执行步骤。

## 项目上下文

涉及项目代码时，先读取框架 `registry/project-context.yaml` 中的项目背景、根路径和已登记的按需扩展信息，再读取与任务匹配的 `required_context.documents`，最后按需读取其他 `business_docs`。项目业务规则、词汇表、接口说明和项目专属规范在项目自身 `docs/` 下维护；框架关联表只保存背景、加载触发条件、工作定位，以及实际需要的仓库、运行、集成或运维入口。已登记的项目路径必须使用绝对路径，不要求项目填写不适用的字段。涉及业务概念时，优先使用项目词汇表中的标准术语；发现新术语或歧义时先登记，不在接口和任务中混用同义词。
