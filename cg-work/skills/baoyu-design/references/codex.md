...
| `copy_starter_component` | Shell `cp starter-components/<file> local/projects/<project-id>/design/<project>/` (or read and adapt). |
...
Start or reuse one server for the whole `local/projects/<project-id>/design/` directory:
...
Use screenshots when visual layout matters. Save screenshots under the project's `local/projects/<project-id>/design/<project>/` folder or a temp path, then embed the absolute screenshot path if the user should see it.
...
- Still start the project-design HTTP server.
