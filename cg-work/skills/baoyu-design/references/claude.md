...
| `copy_starter_component` | `Bash cp starter-components/<file> local/projects/<project-id>/design/<project>/` (or `Read` + adapt) |
...
Serve the whole `local/projects/<project-id>/design/` directory once (one server for all projects) and reuse it. Preview through the Claude Preview MCP, which serves from a named config in `.claude/launch.json`: define a single project-design server that serves the whole `local/projects/<project-id>/design/` directory (`python3 -m http.server 4311 --directory local/projects/<project-id>/design`) so every project shares one server.
...
**If the preview MCP is unavailable,** fall back by file type. A fully self-contained single file can be opened with `open <path>` (`file://`); a multi-file prototype (`<script src="…jsx">`) will NOT load over `file://` and needs HTTP — start the project-design server yourself (`python3 -m http.server 4311 --directory local/projects/<project-id>/design`) and open the URL, or spawn an `Agent` to verify. Never leave the user on a view that silently failed to load its components.
...
1. **Serve the deck over HTTP** — the CLI needs an `http(s)` URL, not `file://` (deck-stage and multi-file decks require a served origin). Reuse the one `designs` server (`python3 -m http.server 4311 --directory local/projects/<project-id>/design`); the deck is then at `http://localhost:<port>/<project>/<file>.html`.
2. **Write the gen_pptx input object to a JSON file** (e.g. `/tmp/<project>-pptx.json`) — same schema as the export docs.
3. **Run the CLI:**

   ```bash
   node <skill>/agents/gen-pptx/dist/cli.mjs --url <servedDeckUrl> --config <jsonPath> --out local/projects/<project-id>/design/<project>
   ```
...
1. **Serve the animation over HTTP** — the CLI needs an `http(s)` URL, not `file://` (multi-file `<script src>` animations require a served origin). Reuse the one `designs` server (`python3 -m http.server 4311 --directory local/projects/<project-id>/design`); the page is then at `http://localhost:<port>/<project>/<file>.html`. The CLI appends the capture-mode query param itself.
...
3. **Run the CLI:**
   ...
   ```bash
   node <skill>/agents/gen-video/dist/cli.mjs --url <servedUrl> --config <jsonPath> --out local/projects/<project-id>/design/<project>
   ```
