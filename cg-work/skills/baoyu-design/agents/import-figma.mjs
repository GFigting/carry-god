...
async function cmdOutline(positional, flags) {
  ...
  out.push(`  full system  node ${selfScript()} design-system ${sh(fileArg)} local/projects/<project-id>/design/<slug>`);
...
async function cmdDesignSystem(positional, flags) {
  const [fileArg, destArg] = positional;
  if (!fileArg || !destArg) usage('design-system needs a .fig file and a destination folder (e.g. local/projects/<project-id>/design/<slug>)');
  const { fig, file } = await loadFig(fileArg);
