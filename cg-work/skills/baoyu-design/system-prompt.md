...
## cg-work storage policy

When operating inside cg-work, obtain `<project-id>` from `local/projects/<project-id>/project-context.yaml`. Store all deliverables and associated design-system files under `local/projects/<project-id>/design/<project-name>/`; do not use a repository-root `designs/` directory. If no valid project context exists, ask for the project id before creating files.
...
- If none of the above matches but you are in a Claude Desktop-like or other file-capable harness that can read/write files and run a shell, continue with the generic workflow: ask questions in chat, serve `local/projects/<project-id>/design/` over HTTP, and give the user the local file path plus URL.
...
- Save each user-facing deliverable into the project's `local/projects/<project-id>/design/<project-name>/` folder. Keep support files (CSS, research notes) alongside it.
...
- For a regular project, also confirm **where to save it** (default `local/projects/<project-id>/design/<slug>/`) and **which design system(s) to use**: discover the repo's systems with `glob local/projects/<project-id>/design/*/_ds_manifest.json` and present them as a multiSelect (none / one / several); if one is picked, offer its starting points as seeds. See `built-in-skills/use-design-system.md`.
...
## Starter Components
Ready-made HTML/JS/JSX scaffolds live in the `starter-components/` directory next to this file — use them instead of hand-drawing device frames, deck shells, canvases, or animation timelines. To use one, copy it into your project (`cp starter-components/<file> local/projects/<project-id>/design/<project>/`) or read it and adapt; each file carries its own usage notes at the top.
