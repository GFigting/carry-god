...
Everything lives flat under `local/projects/<project-id>/design/`. The two kinds are told apart by a root marker file, not by folder name:

- **A design system** has `_ds_manifest.json` at its **root** — `local/projects/<project-id>/design/<ds>/_ds_manifest.json` (the compiler writes it there). It is self-describing; it does **not** carry a `_d_meta.json`.
- **A regular project** has `_d_meta.json` at its **root**, plus — for each system it consumes — a `local/projects/<project-id>/design/<project>/_ds/<slug>/` folder whose own `_ds_manifest.json` sits **one level deeper**. (The `_d_` prefix — matching the `_ds_` marker convention — keeps our metadata from colliding with any `meta.json` the deliverable itself ships at the project root.)

So `glob local/projects/<project-id>/design/*/_ds_manifest.json` matches **only authored design systems**, never a consumed copy (those live at `local/projects/<project-id>/design/*/_ds/*/_ds_manifest.json`). This holds even when a project consumes several systems.
...
Run `glob local/projects/<project-id>/design/*/_ds_manifest.json`. For each hit, read enough to describe it in a question:
...
1. **Where to save** — default `local/projects/<project-id>/design/<descriptive-slug>/`; allow a custom location.
...
`<skill>` is this skill's directory; `<dsDir>` is the design-system folder (e.g. `local/projects/<project-id>/design/fluent2-design-system`); `<projectDir>` is your project folder. Pass `--primary` for the system the user chose as primary (otherwise the **first** system imported auto-claims the primary slot, and later imports leave it untouched).
...
- **Mine it for what you need.** Copy out the fonts and colors you use; for prototypes and designs, copy out any relevant components. If the system ships mocks of existing products and you're asked to design something similar, **fork those mocks** to start — it beats designing from scratch. The runtime copy under `_ds/<slug>/` holds the CSS + compiled bundle; the system's **source** (its `sourcePath` in `_d_meta.json`, e.g. `local/projects/<project-id>/design/<ds>/ui_kits/`, `preview/`, component files) is where the mocks, specimens, and component source live — read and fork from there.
  ...
  "designSystems": [
    {
      ...
      "sourcePath": "local/projects/<project-id>/design/fluent2-design-system"
    }
  ],
