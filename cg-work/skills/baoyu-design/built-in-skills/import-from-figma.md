...
- **Design system** — emit every component + variables into a new `local/projects/<project-id>/design/<slug>/` folder that follows [design-system-authoring-guide.md](design-system-authoring-guide.md), then compile/check/preview as usual.
...
1. **Mount** the decoded file into the project, then explore with Read/Grep/Glob:
   ...
   - Authoring a design system and want a reference mount too? Mount **outside** the DS folder (e.g. `local/projects/<project-id>/design/_sources/<slug>-fig/`) — the compiler scans the whole tree and would otherwise bundle the mounted JSX.
...
1. Emit everything in one shot:
   ```bash
   node <skill>/agents/import-figma.mjs design-system <file.fig> local/projects/<project-id>/design/<slug> --name "Display Name" [--pages <a,b>]
   ```
...
node import-figma.mjs design-system <file.fig> <local/projects/<project-id>/design/slug> [--name "Title"] [--pages <a,b>] [--force]
```
