...
```
node <skill>/agents/compile-design-system.mjs local/projects/<project-id>/design/<project>
```

(`<skill>` is this skill's directory; `local/projects/<project-id>/design/<project>` is the design-system folder.) The compiler discovers everything from **file content and sibling relationships — not from folder names** — so the only fixed location is the global-CSS entry point (below). It bundles the components into a runtime library, indexes the styles, and writes the three artifacts.
...
```
node <skill>/agents/check-design-system.mjs local/projects/<project-id>/design/<project>
```
...
```
node <skill>/agents/build-preview.mjs local/projects/<project-id>/design/<project>
```
