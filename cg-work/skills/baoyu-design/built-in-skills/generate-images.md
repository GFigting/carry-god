...
- Save kept images inside the project (`local/projects/<project-id>/design/<project>/imgs/`), with prompt files in `local/projects/<project-id>/design/<project>/prompts/`, so deliverables stay self-contained. Some flows have their own convention (a mobile prototype's icon is `icon.png` in the project root) — follow the calling flow.
