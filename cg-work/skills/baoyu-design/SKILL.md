...
For a cg-work project, derive `<project-id>` from `local/projects/<project-id>/project-context.yaml` and store every design deliverable, copied asset, design-system source, and preview screenshot under `local/projects/<project-id>/design/<descriptive-project-name>/`. Do not create or use a repository-root `designs/` directory. If the project id is unavailable, ask for it before creating a design folder. This policy overrides the upstream default location while preserving all other design-methodology instructions.
...
- Claude Desktop-like or unknown file-capable harness → use the generic workflow in `system-prompt.md`; ask questions in chat, write files normally, serve `local/projects/<project-id>/design/` over HTTP, and tell the user the local file path + URL.
